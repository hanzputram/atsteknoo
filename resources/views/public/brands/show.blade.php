...
@section('title', ($brand->meta_title ?: $brand->name) . ' - Distributor Resmi PT. Anugerah Tama Sejati')
@section('meta_description', $brand->meta_description ?: 'Katalog produk resmi brand ' . $brand->name . ' dari PT. Anugerah Tama Sejati.')
...
<div class="bg-slate-50 py-10 sm:py-14 border-b border-slate-200/80">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="flex items-center gap-2 text-xs text-slate-500 mb-4" aria-label="Breadcrumb">
            ...
            <span>&rsaquo;</span>
            <a href="{{ route('brands.index') }}" class="hover:text-rose-600 transition">Prinsipal Brand</a>
            <span>&rsaquo;</span>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-6">
                ...
                <div class="w-24 h-24 rounded-2xl bg-white p-3 border border-slate-200 shadow-xs flex items-center justify-center shrink-0">
                    <img src="{{ route('media.view', $brand->logo_id) }}" alt="{{ $brand->name }}" class="max-h-full max-w-full object-contain">
                </div>
                @endif
                <div>
                    ...
                    <p class="text-sm text-slate-600 mt-1 max-w-2xl">
                        Katalog resmi dan spesifikasi teknis produk {{ $brand->name }}.
                    </p>
                </div>
            </div>
